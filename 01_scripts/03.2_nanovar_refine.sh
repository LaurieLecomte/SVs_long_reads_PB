...
# manitou
# parallel -a 02_infos/ind_PB.txt -j 4 srun -c 10 -p medium --time=3-00:00:00 -J 03.2_nanovar_refine_{} --mem=80G -o log/02.2_nanovar_refine_{}_%j.log /bin/sh ./01_scripts/03.2_nanovar_refine.sh {} &
...
# 3. Refine with iris
iris genome_in=$GENOME vcf_in=$CALLS_DIR/nanovar/$SAMPLE/"$SAMPLE"_PASS_correctedDUPs_dupToIns.vcf reads_in=$BAM vcf_out=$CALLS_DIR/nanovar/"$SAMPLE"_PASS_refined_dupToIns.vcf --out_dir=$CALLS_DIR/nanovar/$SAMPLE --keep_long_variants --also_deletions --threads=$CPU
